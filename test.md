...
3. The student should see the prompt: *"Does this answer your question?"*
4. Verify that:
...
2. If a similar question is found, the system should display a notification such as: *"A similar question already exists in the Community Questions section."*
3. The student should then be asked: *"Do you want to proceed with asking the same question?"*
...
*(2 failures caused by API rate-limit fallback to mock mid-test — not a model quality issue)*
...
const SEMANTIC_THRESHOLD = 0.50;  // lower to catch more (risk: noise); raise to filter more (risk: misses)
...
*Test script location: `apps/server/src/scripts/test-search-accuracy.ts`*
*Source data: `troubleshoot_summership_2026_questions.md`, `apps/server/src/scripts/seed-faqs.ts`*
