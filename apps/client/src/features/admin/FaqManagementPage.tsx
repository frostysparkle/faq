    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 18, marginBottom: 16 }}>
      ...
      <div>
        ...
        <div style={{ marginTop: 8, fontSize: 12, color: 'var(--color-text-muted)' }}>
          This week:{' '}
          <strong style={{ color: 'var(--color-text)', fontWeight: 700 }}>{thisWeek}</strong>
        </div>
      </div>
    </div>
  );
...
function TrendBox({ color, points }: { color: string; points?: number[] }) {
  return (
    ...
    >
      <div
        style={{ fontSize: 11, color: 'var(--color-text-muted)', fontWeight: 500, marginBottom: 8 }}
      >
        Trend (last 7 days)
      </div>
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
        {dayLabels.map((label, i) => (
          <span key={i} style={{ fontSize: 9, color: 'var(--color-text-muted)', minWidth: 0 }}>
            {label}
          </span>
        ))}
      </div>
