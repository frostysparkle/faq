...
import { Archive, Bot, ChevronDown, ChevronUp, MessageSquare, ThumbsDown, ThumbsUp, Trash2 } from 'lucide-react';
...
export function ChatbotFeedbackPage() {
  ...
  const FILTER_LABELS: Record<Filter, string> = { all: 'All', helpful: 'Helpful', unhelpful: 'Unhelpful', archived: 'Archived' };
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 20 }}>
        <div className="mod-card mod-card-blue" style={{ padding: '18px 20px', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--color-text)', marginTop: 3 }}>Total Feedback</div>
            <div style={{ fontSize: 11, color: 'var(--color-primary)', fontWeight: 600, marginTop: 2 }}>Active only</div>
          </div>
        </div>
          ...
          {!list.isLoading && (list.data?.length ?? 0) === 0 && (
            <div style={{ textAlign: 'center', padding: '32px 0' }}>
              <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'var(--color-primary-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}>
                {filter === 'archived' ? <Archive size={24} color="var(--color-primary)" /> : <Bot size={24} color="var(--color-primary)" />}
              </div>
...
const STATUS_META: Record<PublicChatFeedback['status'], { label: string; color: string; bg: string }> = {
...
function StatusBadge({ status }: { status: PublicChatFeedback['status'] }) {
  ...
  return (
    <span style={{
      ...
      color: m.color, background: m.bg, textTransform: 'uppercase', letterSpacing: '0.06em',
    }}>
      {m.label}
    </span>
  );
...
function FeedbackRow({ row, onMutate }: { row: PublicChatFeedback; onMutate: () => void }) {
  ...
  return (
    <div style={{ background: 'var(--color-card)', borderRadius: 14, border: `1px solid var(--color-border)`, overflow: 'hidden', opacity: isBusy ? 0.7 : 1, transition: 'opacity 0.15s' }}>
      {/* Header */}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              ...
              <span style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>by {row.user.name} · {timeAgo(row.createdAt)}</span>
            </div>
          </div>
            ...
            {isAdmin && (
              <button
                ...
                style={{
                  ...
                  width: 30, height: 30, borderRadius: 8, cursor: 'pointer', border: '1px solid var(--color-border)',
                  background: 'transparent', color: 'var(--color-danger)', flexShrink: 0, fontFamily: 'inherit',
                }}
              >
                <Trash2 size={13} />
              </button>
