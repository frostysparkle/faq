  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
        ...
        <span style={{ fontSize: 19, fontWeight: 800, color: 'var(--color-text)', letterSpacing: '-0.02em' }}>My Questions</span>
      </div>
      ...
      {isLoading && <div className="mod-card mod-card-purple" style={{ padding: 20, color: 'var(--color-text-muted)', fontSize: 13 }}>Loading…</div>}

      {!isLoading && (data?.length ?? 0) === 0 && (
        <div className="mod-card mod-card-purple" style={{ padding: 36, textAlign: 'center' }}>
          <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'var(--color-purple-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px' }}>
            <HelpCircle size={24} color="var(--color-purple)" />
          </div>
          <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 6 }}>No {tab} questions yet</div>
          <div style={{ fontSize: 13, color: 'var(--color-text-muted)' }}>
            {tab === 'personal' ? 'Personal questions go directly to moderators.' : 'Community questions get peer answers — try asking one.'}
          </div>
        </div>
      )}
...
function QuestionRow({ q }: { q: PublicQuestion }) {
  ...
  return (
    <button onClick={() => navigate(`/community/${q.id}`)} style={{ display: 'block', textAlign: 'left', background: 'none', border: 'none', padding: 0, cursor: 'pointer', width: '100%' }}>
...
function StatusTick({ q }: { q: PublicQuestion }) {
  if (q.type === 'personal') {
    if (q.displayState === 'responded') return (
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12, fontWeight: 700, color: 'var(--color-success)', background: 'var(--color-success-bg)', borderRadius: 20, padding: '4px 10px', flexShrink: 0 }}>
        <CheckCheck size={13} /> Responded
      </span>
    );
    if (q.displayState === 'seen') return (
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12, fontWeight: 600, color: 'var(--color-primary)', background: 'var(--color-primary-bg)', borderRadius: 20, padding: '4px 10px', flexShrink: 0 }}>
        <CheckCheck size={13} /> Seen
      </span>
    );
    return (
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12, fontWeight: 600, color: 'var(--color-text-muted)', borderRadius: 20, padding: '4px 10px', flexShrink: 0, border: '1px solid var(--color-border)' }}>
        <Check size={13} /> Posted
      </span>
    );
  }
  const map: Record<string, 'accent' | 'warning' | 'success' | 'default'> = { open: 'accent', answered: 'warning', resolved: 'success', duplicate: 'default', archived: 'default' };
