...
export function UnifiedSearchPage() {
  ...
  const { search, clear, results, isReady, indexStats, error } = useUnifiedSearch(
    '/api/help-data/export',
  );
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
        ...
        <div>
          <span style={{ fontSize: 19, fontWeight: 800, color: 'var(--color-text)', letterSpacing: '-0.02em' }}>Unified Search</span>
          <span style={{ fontSize: 13, color: 'var(--color-text-muted)', marginLeft: 10 }}>FAQs &amp; Community Q&amp;A · instant TF-IDF</span>
        </div>
        {indexStats && (
          <span style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--color-text-muted)', display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      <div className="mod-card mod-card-blue" style={{ marginBottom: 16 }}>
        <div style={{ padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
          <Search size={18} color={isReady ? 'var(--color-primary)' : 'var(--color-text-muted)'} style={{ flexShrink: 0 }} />
          <input
            autoFocus
            value={inputValue}
            ...
            style={{
              ...
              color: 'var(--color-text)', fontSize: 15, fontFamily: 'inherit',
              opacity: !isReady && !error ? 0.5 : 1,
            }}
          />
          {!isReady && !error && (
            <span style={{ fontSize: 12, color: 'var(--color-primary)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5 }}>
              <span style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--color-primary)', animation: 'pulse 1.2s ease-in-out infinite' }} />
              Syncing…
            </span>
          )}
          {isReady && !error && (
          ...
          {inputValue && (
            <button type="button" onClick={() => handleInput('')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-muted)', padding: 4, display: 'flex', alignItems: 'center' }}>
              <X size={16} />
            </button>
          )}
        </div>
      ...
      {!hasQuery && isReady && !error && (
        <div className="mod-card mod-card-blue" style={{ padding: 40, textAlign: 'center' }}>
          <div style={{ width: 56, height: 56, borderRadius: '50%', background: 'var(--color-primary-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px' }}>
            <Search size={26} color="var(--color-primary)" />
          </div>
          <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 6 }}>Start typing to search</div>
          <div style={{ fontSize: 13, color: 'var(--color-text-muted)', maxWidth: 360, margin: '0 auto' }}>
            Searches {indexStats?.doc_count ?? '…'} articles across FAQs and Community Q&amp;A simultaneously
          </div>
          <div style={{ marginTop: 16, display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
            {['NOC certificate', 'team formation', 'attendance', 'phase 2', 'mentor'].map((q) => (
              <button key={q} onClick={() => handleInput(q)} style={{
                fontSize: 12, fontWeight: 600, padding: '5px 14px', borderRadius: 20,
                border: '1.5px solid var(--color-border)', background: 'var(--color-pill)',
      ...
      {hasQuery && isReady && totalResults === 0 && (
        <div className="mod-card mod-card-orange" style={{ padding: 36, textAlign: 'center' }}>
          <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 6 }}>No results for "{inputValue}"</div>
      ...
      {hasQuery && totalResults > 0 && (
        <>
          <div style={{ fontSize: 13, color: 'var(--color-text-muted)', marginBottom: 12, fontWeight: 600 }}>
            ...
            {' · '}
            <span style={{ color: 'var(--color-primary)' }}>{results.community.length} Community</span>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                <div style={{ width: 30, height: 30, borderRadius: 9, background: 'var(--color-success-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <BookOpen size={15} color="var(--color-success)" />
                </div>
                <span style={{ fontSize: 15, fontWeight: 800, color: 'var(--color-text)', letterSpacing: '-0.01em' }}>
                  Official FAQs
                  <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--color-text-muted)', marginLeft: 8 }}>{results.faqs.length}</span>
                </span>
              </div>

              {results.faqs.length === 0 ? (
                <div className="mod-card mod-card-green" style={{ padding: '20px 18px', textAlign: 'center' }}>
                  <div style={{ fontSize: 13, color: 'var(--color-text-muted)' }}>No FAQ matches</div>
                </div>
              ) : (
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                <div style={{ width: 30, height: 30, borderRadius: 9, background: 'var(--color-primary-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <MessageSquare size={15} color="var(--color-primary)" />
                </div>
                <span style={{ fontSize: 15, fontWeight: 800, color: 'var(--color-text)', letterSpacing: '-0.01em' }}>
                  Community Q&amp;A
                  <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--color-text-muted)', marginLeft: 8 }}>{results.community.length}</span>
                </span>
              </div>

              {results.community.length === 0 ? (
                <div className="mod-card mod-card-blue" style={{ padding: '20px 18px', textAlign: 'center' }}>
                  <div style={{ fontSize: 13, color: 'var(--color-text-muted)' }}>No community matches</div>
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {results.community.map((doc) => (
                    <SearchResultCard key={doc.id} doc={doc} onClick={() => {
    ...
    >
      <div className={`mod-card ${cardClass}`} style={{ padding: '14px 16px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 8 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--color-text)', lineHeight: 1.4, flex: 1 }}>
            {doc.title}
          </div>
          <span style={{
        ...
        {/* Content preview */}
        <div style={{
          fontSize: 12, color: 'var(--color-text-muted)', lineHeight: 1.55, marginBottom: 10,
        ...
        {doc.tags.length > 0 && (
          <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', alignItems: 'center' }}>
            {doc.tags.slice(0, 4).map((tag) => (
              <span key={tag} style={{ fontSize: 10, padding: '2px 7px', borderRadius: 6, background: 'var(--color-pill)', color: 'var(--color-pill-text)', fontWeight: 600 }}>
                #{tag}
              </span>
            ))}
          </div>
