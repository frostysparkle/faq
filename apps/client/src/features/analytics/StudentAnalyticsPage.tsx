...
export function StudentAnalyticsPage() {
  ...
  return (
    <div>
      ...
      {lbLoading && (
        <div className="mod-card" style={{ padding: 32, fontSize: 13, color: 'var(--color-text-muted)', textAlign: 'center' }}>Loading leaderboard…</div>
      )}
      {leaderboard && (
        <KaggleLeaderboard
...
type Entry = { rank: number; userId: string; name: string; spurtiPoints: number; approvedAnswers: number; isMe?: boolean };

function KaggleLeaderboard({ entries, myRank, currentUserId, currentUserName, range }: {
  entries: Entry[];
  myRank?: number;
  ...
  return (
    <div className="mod-card" style={{ overflow: 'hidden', border: '1.5px solid var(--color-border)' }}>

      {/* Podium — top 3 */}
      {podiumSlots.length > 0 && (
        <div style={{ background: 'linear-gradient(180deg, rgba(139,92,246,0.05) 0%, transparent 100%)', padding: '32px 24px 0', borderBottom: '1px solid var(--color-border)' }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'center', gap: 20 }}>
            {podiumSlots.map(({ entry: e, podiumRank, height }) => {
              ...
              const podiumColor =
                podiumRank === 1 ? 'linear-gradient(180deg, #fde68a 0%, #f59e0b 100%)' :
                podiumRank === 2 ? 'linear-gradient(180deg, #e2e8f0 0%, #94a3b8 100%)' :
                                   'linear-gradient(180deg, #fde7c4 0%, #b45309 100%)';

              return (
                <div key={e.userId} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 160 }}>
                  {/* Crown for 1st */}
                  {isFirst && (
                  ...
                  <div style={{
                    ...
                    fontSize: isFirst ? 28 : 22, fontWeight: 900, color: 'white',
                    boxShadow: isFirst ? '0 8px 28px rgba(124,58,237,0.30)' : '0 4px 12px rgba(0,0,0,0.12)',
                  ...
                  {/* Score */}
                  <div style={{ fontSize: isFirst ? 18 : 15, fontWeight: 900, color: 'var(--color-text)', marginBottom: 14 }}>
                    {getScore(e)}{' '}
                    <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--color-text-muted)' }}>{scoreLabel}</span>
                  </div>
                  ...
                  <div style={{
                    ...
                    <span style={{ fontSize: isFirst ? 38 : 28, lineHeight: 1 }}>{medalFor(podiumRank)}</span>
                  </div>
                </div>
              );
            })}
      ...
      {/* Column headers */}
      <div style={{ display: 'grid', gridTemplateColumns: '52px 1fr 200px 90px', alignItems: 'center', padding: '10px 20px', borderBottom: '1px solid var(--color-border)', background: 'var(--color-input)' }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', textAlign: 'center' }}>Rank</div>
        <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Participant</div>
        <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Score</div>
        <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', textAlign: 'right' }}>
      ...
      {entries.map((e) => {
        ...
        return (
          <div key={e.userId} style={{
            display: 'grid', gridTemplateColumns: '52px 1fr 200px 90px', alignItems: 'center',
            padding: '11px 20px',
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              {e.rank <= 3 ? (
                ...
                <span style={{ fontSize: 13, fontWeight: 800, color: 'var(--color-text-muted)', fontVariantNumeric: 'tabular-nums' }}>#{e.rank}</span>
              )}
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
              <div style={{
                ...
                background: isCurrentUser ? 'linear-gradient(135deg, #8b5cf6, #6d28d9)' : rankAvatarBg(e.rank),
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              ...
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: isCurrentUser ? 700 : 500, color: isCurrentUser ? 'var(--color-purple)' : 'var(--color-text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {isCurrentUser ? `${e.name} (You)` : e.name}
                </div>
                <div style={{ fontSize: 11, color: 'var(--color-text-muted)', marginTop: 1 }}>{getSecondary(e)}</div>
              </div>
            </div>
              ...
              <div style={{ height: 7, borderRadius: 4, background: 'var(--color-border)', overflow: 'hidden' }}>
                <div style={{
                  ...
                  background: isCurrentUser
                    ...
                    : e.rank === 1 ? 'linear-gradient(90deg, #fbbf24, #d97706)'
                    : e.rank === 2 ? 'linear-gradient(90deg, #94a3b8, #64748b)'
                    : e.rank === 3 ? 'linear-gradient(90deg, #c97b3a, #92400e)'
            ...
            {/* Score value */}
            <div style={{ textAlign: 'right', fontSize: 14, fontWeight: 800, color: isCurrentUser ? 'var(--color-purple)' : 'var(--color-text)', fontVariantNumeric: 'tabular-nums' }}>
              {score}{' '}
              <span style={{ fontSize: 10, fontWeight: 600, color: 'var(--color-text-muted)' }}>{scoreLabel}</span>
            </div>
          </div>
        );
      })}
      ...
      {myRank && myRank > 20 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 20px', background: 'rgba(139,92,246,0.06)', borderTop: '2px dashed var(--color-border)' }}>
          <div style={{ width: 34, height: 34, borderRadius: '50%', background: 'linear-gradient(135deg, #8b5cf6, #6d28d9)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800, color: 'white', flexShrink: 0 }}>
            {currentUserName.charAt(0).toUpperCase()}
          </div>
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--color-purple)' }}>{currentUserName} (You)</div>
            <div style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>Your current rank: #{myRank}</div>
          </div>
        </div>
      )}
    </div>
