...
const COMMUNITY_QUESTIONS: { title: string; description: string }[] = [
  ...
  { title: 'Portal shows Interview Interrupted after Yaksha', description: 'Yaksha said my interview was completed successfully, but the portal shows "Interview Interrupted". Which status should I trust?' },
  ...
  { title: 'No ViBe course link in offer letter email', description: 'My offer letter arrived but there is no link to the Phase 1 / Bronze course on the ViBe platform. When will I receive the course access link?' },
  ...
  { title: 'Leave policy — weekends and emergency situations', description: 'How many leaves are we allowed during the internship? Are weekends counted as working days? Is there any provision for emergency or medical leave?' },
  { title: 'Is the internship self-paced or fixed schedule', description: 'I want to manage the internship alongside my college timetable. Are there mandatory fixed session timings, or can I complete tasks at my own pace?' },
  { title: 'Are daily sessions on Zoom or Samagama dashboard', description: 'I am unsure how daily sessions are conducted. Will I receive a Zoom link each day, or are sessions delivered through the Samagama portal dashboard?' },
  { title: 'Missed orientation — how to catch up', description: 'I missed the orientation session due to a technical issue on my end. Is there a recording I can access? Will missing orientation affect my internship evaluation?' },
  ...
  { title: 'Spurti Points showing zero after internship started', description: 'My internship started but my Spurti Points are showing as zero on the dashboard. Will they be updated retroactively, or is there something I need to do to trigger the update?' },
];
...
async function ensureQuestions(students: Student[]): Promise<void> {
  ...
  for (let i = 0; i < Math.min(TAG_DISTRIBUTION.length, allQuestions.length); i++) {
    ...
    const taggedIds = shuffle(students).slice(0, tagCount).map((s) => s._id);
...
async function seedAnswers(questions: Question[], students: Student[]): Promise<number> {
  ...
  for (const question of questions) {
    ...
    const target = Math.min(
      randInt(ANSWERS_MIN, ANSWERS_MAX),
      eligible.length,
      remaining,
    );
...
async function seedVotes(questions: Question[], students: Student[], topIds: Set<string>): Promise<number> {
  ...
  for (const answer of answers) {
    const isTop = topIds.has(
      questions.find((q) =>
        answers.some((a) => a._id.equals(answer._id) &&
          // find via questionId on the answer itself
          true),
      )?._id.toString() ?? '',
    );

    const questionOfAnswer = questions.find((q) =>
      answers.filter((a) => {
    ...
    const eligible = students.filter(
      (s) =>
        s._id.toString() !== answer.answeredBy.toString() &&
        !alreadyVoted.has(s._id.toString()),
    );
...
async function boostTopQuestions(topIds: Set<string>, students: Student[]): Promise<void> {
  const topAnswers = await AnswerModel.find(
    { questionId: { $in: Array.from(topIds) } },
  )
    .select('+upvotes +downvotes')
    .lean<AnswerDoc[]>();
...
async function seedInteractions(): Promise<void> {
  ...
  if (students.length === 0) {
    logger.error('No @samagama.test student accounts found. Run seed:accounts + seed:student-accounts first.');
